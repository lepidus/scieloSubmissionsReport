...
import('plugins.reports.scieloSubmissionsReport.classes.tasks.SendReportEmail');
...
class TestableSendReportEmail extends SendReportEmail
{
    ...
    {
        $this->sendReport(null, array(), $report);
    }
...
class SendReportEmailSecurityTest extends TestCase
{
    ...
    {
        $source = file_get_contents(dirname(__DIR__) . '/classes/tasks/SendReportEmail.inc.php');
    ...
    public function testGenerationFailureRemovesTemporaryFile(): void
    {
        ...
        $this->assertSame(array(), glob($this->temporaryDirectory . '/*'));
    }
